...
class ConsultationConfirmController extends GetxController {
  ...
  invoice(String room) async {
    ...
    var querySnapshot1 =
        await collectionRef.where('timeslot', isEqualTo: room).get();
    ...
    nogst_tds = languageSettingVersionRef1.data()!['nogstTds'] as int;
    ...
    if (gsttype == 'Biz') {
      double bizz = bizcom;
      btcs = 0;
      btds = (exegst -bizz)* nogst_tds / 100;
      netpayment = exegst - bizz - btds ;
      sac = languageSettingVersionRef1.data()!['sacBizboost'];
    } else {
      ...
      sac = languageSettingVersionRef1.data()!['sacAdvisor'];
    }
    ...
    if (querySnapshot2.size > 0) {
      ...
      return invoiceCollection
          .add({
            ...
            'excludedGstamt': double.parse(exegst.toStringAsFixed(2)),
            'includedGstamt': double.parse(ingst.toStringAsFixed(2)),
            'createdAt': now,
            'advisorId': adid,
            'timeslot': room,
            'financialYear': fy,
            ...
            'bizGstno': bizgstno,
            'tds': double.parse(btds.toStringAsFixed(2)),
            'tcs': double.parse(btcs.toStringAsFixed(2)),
            'cgst': double.parse(cgstamt.toStringAsFixed(2)),
            'sgst': double.parse(sgstamt.toStringAsFixed(2)),
            'igst': double.parse(igstamt.toStringAsFixed(2)),
            'bizCommision': double.parse(bizcom.toStringAsFixed(2)),
            'netPayment': double.parse(netpayment.toStringAsFixed(2)),
            'advisorStatecode': adstcode,
            'userStatecode': usstcode,
            'sac': sac,
            'bizState': bizstate,
            'bizTax': double.parse(biztax.toStringAsFixed(2)),
            'userTax': double.parse((ingst - exegst).toStringAsFixed(2)),
            'gstType': gsttype,
